...
logger = logging.getLogger("webcrawler.cli")
...
def main() -> None:
    ...
    crawl.add_argument(
        ...
        type=float,
        default=1.0,
        help="minimum seconds between requests per origin",
    )
    ...
    try:
        ...
        try:
            if args.command == "crawl":
                ...
                print(
                    f"new documents: {stored}; "
                    f"total corpus: {store.document_count()}; "
                    f"frontier: {store.frontier_summary()}"
                )
            else:
                count = ingest_warc(store, args.url, args.max_records)
